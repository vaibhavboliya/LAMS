...
<div class="centerbox">
    <form action={{route('admin.insertteaches')}} method="POST" class="atform">
        @csrf
        <div class="box">
            <div class = "container-fluid ">
                ...
                <div class="form-group lecture-date">
                    ...
                    <select class="form-control" style="width: 350px" name="teacher_id">
                        @for($i=0;$i<$count;$i++)
                            <option value={{$teacher_id[$i]}}>{{$teacher_name[$i]}}</option>
                        @endfor
                    </select>
                </div>
                <hr>
                <div class="col-md-auto">
                    <input type="text" name="subject_id" hidden readonly value={{$subject_id}}>
                    <input type="text" name="class_id" hidden readonly value={{$class_id}}>
                    <input style="margin-left:10%; margin-right: 30%" type="submit" class="btn btn-success text-white">
                    <a type="button" class="btn btn-warning text-white" href={{route('admin.teaches')}} >cancel</a>
                </div>
            </div>
        </div>
    </form>
