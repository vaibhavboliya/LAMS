...
class TeacherDashboard extends Controller
{
    ...
    public function mark(Request $req)
    {
        ...
        $subject_name = DB::table('subject')->select('subject_name')->where('$suject_id','=',$subject_id);
        foreach ($subject_name as $subject)
        {
            $subject_name = $subject->subject_name;
        }
        return View('MarkAttendance')->with('$subject',$subject_name)->with('class_name',$class_name)->with('time',$time)->with('date',$date);
    }
}
